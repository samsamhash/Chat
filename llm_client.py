...
def call_llm_local(prompt: str, max_new_tokens: Optional[int] = None, temperature: Optional[float] = None) -> str:
    """
    Generate a response from the local model given a single prompt (string).
    Returns the generated text (string).

    Parameters:
      - prompt: the full prompt to pass to the model (including any system instructions)
      - max_new_tokens: overrides default max tokens (env or fallback)
      - temperature: overrides default temperature
    """
    ...
    if tokenizer is None or model is None:
        # try to reload
        try:
            logger.warning("[LLM] Tokenizer/model not loaded; attempting reload.")
            _load_tokenizer_and_model()
        except Exception as e:
            raise RuntimeError(f"Model not loaded and reload failed: {e}")

    # Hyperparameters (fallbacks)
    max_new_tokens = int(max_new_tokens) if max_new_tokens is not None else LLM_MAX_TOKENS_ENV
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(f"Generation failed: {ex}")
